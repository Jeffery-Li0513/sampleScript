...
def random_vector():
    ...
    if c >= 1.0:  # Guard against very small chance of roundoff erro防止出现c=1的极端情况
        s = 0.0  # Set sin(theta) to zero
    else:
        s = np.sqrt(1.0 - c ** 2)  # Calculate sin(theta) from cos(theta), always positive

    phi = zeta[1] * 2.0 * np.pi  # 从(0,2*pi)范围内随机选择一个角度

    return np.array((s * np.cos(phi), s * np.sin(phi), c), dtype=np.float_)  # s**2 = 1-c**2 => s**2 + c**2 = 1
...
def q_to_a(q):
    ...
    a[0, :] = [q[0] ** 2 + q[1] ** 2 - q[2] ** 2 - q[3] ** 2, 2 * (q[1] * q[2] + q[0] * q[3]),
               2 * (q[1] * q[3] - q[0] * q[2])]
    a[1, :] = [2 * (q[1] * q[2] - q[0] * q[3]), q[0] ** 2 - q[1] ** 2 + q[2] ** 2 - q[3] ** 2,
               2 * (q[2] * q[3] + q[0] * q[1])]
    a[2, :] = [2 * (q[1] * q[3] + q[0] * q[2]), 2 * (q[2] * q[3] - q[0] * q[1]),
