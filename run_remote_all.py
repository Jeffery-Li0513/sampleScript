...
def check_path(final_dir):
    '''
    检查final_dir路径，如果存在该文件夹则返回False；如果不存在，新建文件夹，返回True。
    :param final_dir:
    :return: 布尔值
    '''
    if not os.path.exists(final_dir):
        os.mkdir(final_dir)                 # 如果输入的路劲文件夹不存在，则新建文件夹
        return True
    else:
        if not os.path.isdir(final_dir):    # 判断是否为目录，如果不是目录就继续执行
            os.remove(final_dir)            # os.remove()用于删除指定路径的文件。如果指定的路径是一个目录，将抛出OSError。用来移除文件
            return check_path(final_dir)    # 重新运行一遍，这时已经移除了名字相同的文件，再次运行回新建一个文件夹
        else:
            return False                    # 已经存在该文件夹


def replace_local2remote():
    filen = os.getcwd()                     # 返回当前工作目录
...
def _rename_htmlconf(head_path, fn: str):
    edfp = os.path.join(head_path, fn)
    gogalfp = os.path.join(head_path, fn[1:])
    try:
        os.rename(edfp, gogalfp)
        print("Rename %s to %s done!" % (edfp, gogalfp))
...
if __name__ == '__main__':
    ...
    dd = os.path.dirname(__file__)                  # 当脚本用相对路径运行时，返回空字符串；当以绝对路径运行时，返回所在路径
    final_top_dir = os.path.join(dd, 'remote_sv')
    en_path = os.path.join(final_top_dir, 'en')
    cn_path = os.path.join(final_top_dir, 'zh_CN')
    
    if check_path(final_top_dir):
        '''
        shutil.copytree(old_path, new_path) 文件夹复制
        new_path的路径末尾必须要加上粘贴后新的文件夹名，且执行前Test文件夹底下不能有data文件夹，不然会报错提示已经存在data文件夹。
        '''
        shutil.copytree(os.path.join(dd, 'source', 'en', 'build', 'html'), en_path)
        shutil.copytree(os.path.join(dd, 'source', 'zh_CN', 'build', 'html'), cn_path)
    
    replace_local2remote()
    for mm in [en_path, cn_path]:
        for i in ['_static', '_images', '_sources', '_downloads']:
            _rename_htmlconf(mm, i)
